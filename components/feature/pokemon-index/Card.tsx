...
export const Card = ({ pokemonId }: Props) => {
  ...
  const { pokemon, pokemonJaName, isLoading } = useGetPokemonNames(pokemonId);
  ...
  if (!pokemon || !pokemonJaName) return null;

  return (
    <Link
      href={`/pokemon/${pokemon.id}`}
      className="relative flex aspect-square w-full flex-col items-center justify-center rounded-lg bg-gray-200 text-center text-inherit transition-colors dark:bg-gray-700 dark:hover:bg-gray-600"
    >
